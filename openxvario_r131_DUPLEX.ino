...
/* JETIDUPLEX Version 10.3.2013   (based on openXvario version r131)                   */
/* By "define JETIDUPLEX" a DUPLEX EX compatible data stream is created at the HW UART */
/* with the channels Spannung, Temperatur, Relativhöhe, Steigrate, further a classic   */
/* Jetibox textframe with the same data. No Jetibox keyboard supported so far.         */
/* REMARK: */
...
/***************************************************************************************/
#define JETIDUPLEX                  // Jeti Duplex EX Protocol
#define DUOLED                      // DuoLED on myVarioEX-Board (Arduino Pins 8 and 9)
#define MYVARIOEX                   // Different Voltage Divider for Lipo on Custom Vario Board
...
void setup()
{
  ...
  {
    JetiSensor ("Jeti myEX Vario ","Ver. 10.03.2013 ",0,false);
    delay(80);
  }
#endif // JETIDUPLEX
...
/******************************************************************/
    Relalt = alt - Zeroalt;                               // relative altitude above ground

    // Write 2 lines to the Jetibox Classic Terminal
    strcpy (JetiLine1, "  0.00V     2.0m");              // Zeile 1: Spannung und rel. Höhe
    Dezipunkt_Zahl (JetiLine1, LipoVoltage/10, 0) ;
    Dezi_Integer (JetiLine1, (uint16_t)Relalt/10, 9);
    strcpy (JetiLine2, "  0.0\xDF\x43      m/s");        // Zeile 2: Temperatur [°C] und Steigrate [m/s]
    Dezi_Integer99 (JetiLine2, Temp, 0);
...
void SendEx (uint8_t x)
{
  ...
  case 11:
    ex[2] = 0x00;						// 2Bit Type(0-3) 0x40=Data, 0x00=Text
    ex[8] = 0x05;						// Identifier (5 = Kanalname)
...
//    Volt = 126;					// Testwert 12.6V (Dummy, da aktuell noch keine A/D-Abfrage)
    ...
    ex[n++] = uint14 ((uint16_t)alt/10) ;             	// 6-Bit-HighByte Höhe (Komma an vorletzter Stelle)

    ex[n++] = 0x31;					// Identifier = 3 (Kanal 3= Höhe) + Data Type 1 (int14_t)
    ex[n++] = ((uint16_t)Relalt/10) & 0xFF;      	// LowByte Höhe
    ex[n++] = uint14 ((uint16_t)Relalt/10) ;            // 6-Bit-HighByte Höhe (Komma an vorletzter Stelle)
    ...
    ex[n++] = uint14 (climbRate/10);                  	// HighByte Steigrate (Komma-Pos an vorletzter Stelle)
  }                                                     // in "n" bleibt die genutzte Paketlänge stehen

  jeti_crypt (ex, n) ;		                       // complete and crypt the EX frame 

  ack = JetiGetChar();                                 // read keyboard of Jetibox / DC16 transmitter, dont evaluate it up to now
